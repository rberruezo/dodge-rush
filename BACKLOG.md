...
| `GME-003` | Testear ContinueScene: el timer de 6s auto-declina correctamente + revisar legibilidad para kids (ver notas Game Designer) | P1 | DONE (2026-06-26) | Mobile Game Designer |
...
| `GME-012` | **Fairness de scoring entre modos (Relax vs Classic)**: en Relax la velocidad es menor, los gaps más anchos y la ramp más lenta → el mismo tiempo sobrevivido genera menos presión pero acumula igual score de supervivencia. Opciones: (a) aplicar un multiplicador de penalización al score en Relax (ej. `×0.7`); (b) separar el high score por modo (Classic high ≠ Relax high), que es la opción más limpia para kids que juegan en Relax; (c) mostrar el modo junto al score en pantallas de resultado. **Decisión de diseño requerida antes de lanzar** — sin esto el leaderboard/récord no tiene contexto. | P1 | PENDING | — |
