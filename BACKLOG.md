...
2. **Mecánica riesgo↔recompensa: obstáculos con dos gaps (fácil vs difícil), el difícil paga más** (GME-017).
...
| `SKN-007` | **[PRE-STORE #1] Revertir el character a la versión anterior más simple que se veía bien.** El sprite Tier-3 actual "lee más cargado"; volver al look previo más limpio. Recuperar el sheet/commit anterior (`git log -- public/assets/character.png`), reemplazar el actual y validar facing (`PlayerFacing.ts`) + las 12 poses. | **P0** *(pre-store)* | **DONE** (2026-06-29) | SKN-007 character sprite adjustment |
