 ...
 * [OBS-008] These match the modular atlas produced by scripts/pack-obstacles.py:
 * each tile is composed as [mirrored cap-end | body column | cap-end] from the
 * separate cap/centre art in art-src/obstacles/pieces/. Every tile is 125x88, so
 * the existing 3-slice pipeline (capFraction 0.32) lands `_l`/`_r` exactly on the
 * decorated gap-facing caps and `_c` on the seamless body column at `centerX`.
 */
export interface ObstacleFrameDef {
  ...
  // fill a wall's mid-section. pack-obstacles.py picks the brightest, saturated,
  // (near-)opaque column from the *_center piece, so the tiled body stays bright
  // and on-brand (visible on night) while a 1px column tiles with zero seam.
  centerX: number;
}

/**
 * Additional animation frames for obstacles that have >1 sprite frame.
 * Each entry maps `<frame>_f<n>` → atlas rect. The cap animates; the body column
 * is identical across a type's frames so the wall body never flickers.
 */
...
// [OBS-008] Generated by scripts/pack-obstacles.py — do not hand-edit; re-run the
// packer after changing art-src/obstacles/pieces/ and paste its output here.
export const OBSTACLE_ANIM_FRAMES: ObstacleAnimFrameDef[] = [
  { name: 'red_arrow_f1', x: 510, y: 2, width: 125, height: 88, centerX: 572 },
  { name: 'red_arrow_f2', x: 637, y: 2, width: 125, height: 88, centerX: 699 },
  { name: 'red_spike_f1', x: 129, y: 92, width: 125, height: 88, centerX: 191 },
  { name: 'red_spike_f2', x: 256, y: 92, width: 125, height: 88, centerX: 318 },
  { name: 'red_spike_f3', x: 383, y: 92, width: 125, height: 88, centerX: 445 },
  { name: 'gold_block_f1', x: 129, y: 182, width: 125, height: 88, centerX: 191 },
  { name: 'gold_block_f2', x: 256, y: 182, width: 125, height: 88, centerX: 318 },
  { name: 'gold_block_f3', x: 383, y: 182, width: 125, height: 88, centerX: 445 },
  { name: 'gold_block_f4', x: 510, y: 182, width: 125, height: 88, centerX: 572 }
];

export const OBSTACLE_FRAMES: ObstacleFrameDef[] = [
  { name: 'blue_bar', x: 2, y: 2, width: 125, height: 88, centerX: 64 },
  { name: 'green_bar', x: 129, y: 2, width: 125, height: 88, centerX: 191 },
  { name: 'purple_pillar', x: 256, y: 2, width: 125, height: 88, centerX: 318 },
  { name: 'red_arrow', x: 383, y: 2, width: 125, height: 88, centerX: 445 },
  { name: 'red_spike', x: 2, y: 92, width: 125, height: 88, centerX: 64 },
  { name: 'stone_crack', x: 510, y: 92, width: 125, height: 88, centerX: 572 },
  { name: 'blue_tile', x: 637, y: 92, width: 125, height: 88, centerX: 699 },
  { name: 'gold_block', x: 2, y: 182, width: 125, height: 88, centerX: 64 }
];
