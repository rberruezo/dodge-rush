...
export class ShopScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    new Button(this, cx, 898, 'BACK', () => this.scene.start('MainMenu'), {
      width: 260,
      height: 70,
      fontSize: 26,
      fill: 0x44345e
  ...
  private makeCard(skin: SkinDef, x: number, y: number): void {
    ...
    const card = this.add.rectangle(x, y, 196, 150, COLORS.panel, 0.92).setStrokeStyle(4, stroke);

    const preview = this.add.sprite(x, y - 28, skin.sheet, 6).setScale(0.52);
